...
class PromisePool extends EventEmitter {
  constructor(pool, thePromise) {
    ...
    inheritEvents(pool, this, ['acquire', 'connection', 'enqueue', 'release', 'purge']);
  }
  ...
  createConnection() {
    const corePool = this.pool;
    return new this.Promise((resolve, reject) => {
      corePool.createConnection((err, coreConnection) => {
        if (err) {
          reject(err);
        } else {
          resolve(new PromisePoolConnection(coreConnection, this.Promise));
...
class PromisePoolCluster extends EventEmitter {
  constructor(poolCluster, thePromise) {
    ...
    this.Promise = thePromise || Promise;
    inheritEvents(poolCluster, this, ['acquire', 'connection', 'enqueue', 'release', 'purge']);
  }
